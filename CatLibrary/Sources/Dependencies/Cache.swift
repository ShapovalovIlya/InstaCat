...
final class Cache<Key: Hashable, Value> {
    ...
    func insert(_ value: Value, forKey key: Key) {
        ...
        logger?.log(
            level: .debug,
            domain: self,
            event: "Set new cache for key: \(String(describing: key))"
        )
    }
    ...
    func removeValue(forKey key: Key) {
        ...
        logger?.log(
            level: .debug,
            domain: self,
            event: "Remover value for key: \(String(describing: key))"
        )
    }
    ...
    func entry(forKey key: Key) -> Entry? {
        guard let entry = wrapped.object(forKey: WrappedKey(key)) else {
            logger?.log(
                level: .debug,
                domain: self,
                event: "No entry for key: \(String(describing: key))"
            )
            return nil
        }
        ...
        guard dateProvider() < entry.expirationDate else {
            logger?.log(
                level: .debug,
                domain: self,
                event: "Entry for key: \(String(describing: key)) expired."
            )
            removeValue(forKey: key)
            return nil
        }
